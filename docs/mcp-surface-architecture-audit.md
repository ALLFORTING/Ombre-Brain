...
| `boot` | Startup context; model workflow | `pinned_chars=5000`, `max_tokens=8000` | Budget-capped text containing triggers, pinned items, mailbox, feel echo, sessions, todos | Read-oriented composite; records due-trigger observation | HIGH workflow significance despite small schema | `server.py: boot`; README; `CLAUDE_PROMPT.md` |
