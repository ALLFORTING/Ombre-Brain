...
| `boot` | Startup context; model workflow | `pinned_chars=5000`, `max_tokens=12000` | Budget-capped text containing triggers, pinned items, mailbox, feel echo, sessions, todos | Read-oriented composite; records due-trigger observation | HIGH workflow significance despite small schema | `server.py: boot`; README; `CLAUDE_PROMPT.md` |
...
| Optional parameter/default | `breath.max_results=5`, `boot.max_tokens=12000`, `digest.dry_run=True` | Adding an optional parameter is LOW-RISK at protocol level; changing a default is BREAKING or UNKNOWN because semantics change |
