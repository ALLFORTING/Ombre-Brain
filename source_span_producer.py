        ...
        *,
        ...
        try:
            for start, end, segment_text in segments:
                ...
                self.store.create_span_descriptor(
                    revision_id,
                    start,
                    end,
                    span_id=span_id,
                    span_hash=hashlib.sha256(raw_byte_slice(raw_bytes, start, end)).hexdigest(),
                    allow_sealed=allow_sealed,
                    allow_restricted_admin=allow_restricted_admin,
                )
                mapping = self.store.create_candidate_mapping(
                    revision_id=revision_id,
                    run_id=run_id,
                    run_item_key=run_item_key,
                    producer_version=self.producer_version,
