...
- durable import — operation plan 首次创建时固定 ID，并纳入 payload/digest；apply、retry 和 restart 复用原计划。已有目标身份优先复用，计划已提交但 item checkpoint 尚未写入时也不重新规划。旧格式 operation 保留原 payload/digest，在明确 todo 写入时建立缺失身份，重放不更换已落盘 ID。旧计划重放保留已有 completion 和首次 `done_at`，不重新激活已完成身份；不改变启动展示规则。
