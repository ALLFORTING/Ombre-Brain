...
`boot` 接受可选的 `pinned_chars` 和 `max_tokens` 参数：前者控制钉选内容的字符预算，后者控制整体返回的 token 预算（默认及硬上限均为 16000）。婷留言永远在第一块。第二块是增量摘要：它以最后一次成功 boot 的事件高水位为基线，简短列出之后新建、正文修改、todo 状态变化及 superseded/invalidated 标记的可见 bucket；首次显示“暂无上次 boot 基线”，没有变化时显示“无新增变化”。sealed、已删除或其他不可见 bucket 不会出现在该段；摘要最多约 600 token，按时间与重要度稳定选择完整条目，并在需要时显示还有多少项未展开。成功返回后才推进基线，因此失败 boot 不丢失变化。最新的可见 pending note 只会自动递送一次；两封之间没有 boot 时只递送较新的，较旧的保留在历史中但不补投。没有可递送留言时显示最后一封可见实际留言的真实 `created_at`，从未有可见历史时显示“暂无历史留言”。未到 `open_at`、sealed 或被 dismissed 的 note 不泄露。若完整正文无法纳入预算，boot 只提示 `get_note(note_id=...)`，不标记递送。其余各块按“今日触发 → 最新 letter → todos → 最近归档 → 钉选索引 → feel 回声”的顺序出队；发生截断时，返回末尾会区分列出部分截断和完全未输出的块名。预算允许时，最新 letter、todos、最近归档和钉选索引分别预留 1000、1500、1200、4000 字符；实际内容短于保底时，未使用的额度会归还给后续块。具体默认值与边界以 [`docs/mcp-public-contract.json`](docs/mcp-public-contract.json) 为准。

`boot` accepts optional `pinned_chars` and `max_tokens` parameters; the default and hard maximum for `max_tokens` are both 16000. Ting notes are always first. The second section is a delta summary based on the event high-water mark from the last successful boot: it lists compact, visible bucket creation, content, todo-state, and superseded/invalidated changes. Its first run states that no baseline exists; an unchanged run explicitly says there are no changes. Sealed, deleted, and otherwise invisible buckets do not appear. The summary is independently bounded to about 600 tokens and selects complete records in stable time/importance order, naming any remaining records. The checkpoint advances only after a successful boot body is completed. The remaining sections are emitted in this order: daily triggers, latest letter, todos, recent archives, pinned index, and feel echo. Ting-note delivery and truncation behavior remain as described above. See [`docs/mcp-public-contract.json`](docs/mcp-public-contract.json) for the current defaults and bounds.
