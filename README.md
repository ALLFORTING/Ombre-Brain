...
| `breath` | 检索或浮现记忆；`query` 用于定向检索。它是 retrieval-oriented，命中/排序可能更新 activation metadata；当前版本在每条正常结果前显示紧凑的 provenance badge / Retrieval or surfacing; targeted `query` is preferred when the topic is known, bounded activation metadata may be updated, and normal current results carry a compact provenance badge |
...
| `hold` | 写入单条记忆或模型自己的 `feel` 反思；普通写入仅在匹配到规范化正文相同且可复用的桶时复用，不做语义合并；相似提醒与冲突检查不阻止写入 / Store one memory or a model `feel` reflection; ordinary writes reuse an eligible matched bucket only when normalized bodies match, without semantic merging; similarity and conflict warnings do not block the write |
| `grow` | 将日记/长内容拆分并写入记忆桶；每个成功条目返回实际 `bucket_id` 和新建/复用状态，复用仅针对匹配到的相同内容桶 / Digest journal-style content into memory buckets; every successful item reports its actual `bucket_id` and created/reused status, with reuse limited to an eligible matched identical-content bucket |
| `trace` | 混合修改工具：元数据、正文替换/追加、todo provenance、related、superseded_by、merge、seal 以及 `delete=True`。delete 必须先取得并回传短时、一次性的 confirm_token；明确成功才代表写前快照成功；没有 MCP undo/restore 命令 / Mixed mutation tool; delete requires a short-lived, one-shot confirm_token before the write-ahead snapshot and deletion can execute; there is no MCP undo/restore command |
...
| `todos` | 汇总未 resolved 且非 sealed 桶中的 todos；`include_provenance=True` 才按明确来源分组显示 / Summarize todos from unresolved, non-sealed buckets; explicit provenance grouping is opt-in |
...
`boot(profile="talk" | "code" | "tg")` 默认使用 `talk`，并在输出顶部标识当前 profile。三个 profile 共享 sealed、superseded、trigger、婷留言和事实来源；profile 只调节显示块、优先级和预算。`talk` 保留完整当前启动上下文；`code` 以结构化 domain/tags、pinned/protected 和 importance 保留工程信息及全局约束；`tg` 保留核心块、高优先级事项与紧凑输出。增量摘要使用独立的 profile checkpoint，避免一个场景消费后其他场景永久漏掉后续变化。其他 boot 语义保持不变：婷留言只自动投递一次，sealed/open_at 继续存在性隐藏，触发条目只在成功 boot 后写入 seen metadata。
...
boot 的待办段在 talk/code/tg 共享 canonical active projection：包含未 resolved、非 sealed 桶中的活动待办（含 archive），不按年龄、importance 门槛或项目标签过滤。T 按展示 identity 计数：每个活动 `(bucket_id, todo_id)` 一项；同文本的不同活动 ID 分别计数；legacy 无 ID 按 `(bucket_id, canonical todo text)` 去重。completed/dropped identity 不展示，专用 `todos()` 的既有输出格式保持不变。
...
- 正常当前 `breath` 的每个命中会以 `[prov=unknown|summary|inference|system]` 开头。`provenance_kind` 是正文来源/保真度元数据，不是生命周期 type、topic/domain、语义用途，也不证明逐字引用；`quote`/`verbatim` 刻意不受支持。`as_of` 不显示该 badge，因为现有历史只保存正文快照、没有 provenance 快照 / Every normal current hit starts with a compact provenance badge. `provenance_kind` describes body provenance/fidelity, not lifecycle type, topic/domain, semantic purpose, or proof of a verbatim quotation. `quote` and `verbatim` are intentionally unsupported. Historical `as_of` output omits the badge because history stores body snapshots only.
...
- session 默认节选仍取正文前 1200 字符；无 query 且确实截断时给出全文读取提示，带 query 的默认节选沿用 `[显示=原文节选·已截断]` 标记；`mode="full"` 的 query 路径保持原有预算截断行为。`importance_min` 按真实 importance 排序并显示 `重要:n` 与桶类型图标，不显示占位权重。/ Default session previews retain the 1200-character cut; full query mode retains its existing budget behavior.
...
- `merge: str = ""` — 方向固定为 `trace(bucket_id=目标, merge=源)`。首次调用仅预览源/目标、内容 hash、关系重连和删源影响，返回 5 分钟一次性 `confirm_token`；使用相同参数和 token 确认后才执行。执行中途失败会返回持久 `operation_id` 和步骤进度；以相同的 `trace(bucket_id=目标, merge=源)` 续做，不重复拼接已写入的目标正文。不能与 `delete` 同用，源桶不能是 pinned/protected / The first call previews a source-to-target merge; a matching confirmation authorizes execution. Retry the same pair after partial failure to resume its durable operation.
...
- `todo_items: list[object] | None = None` — 与 legacy `todos` 互斥，每项须有非空 `text`，可提供 `said_by`（`ting`、`model`、`system`、`unknown`）、可信的 ISO-8601 `said_at`、`source_bucket` 和可选 `id`。`id` 只能引用当前桶已有 todo 身份，可用于明确保身份改写文本，外来 ID 和同 ID 多文本冲突被拒绝。省略 ID 时复用唯一同文本身份，新项或本次明确 todo 写入涉及的 legacy 项获得服务器生成的 ID；多身份匹配须指定 ID。结构化 trace 回执返回各项 ID。
- stable todo ID — 新持久化 todo 在 `todo_provenance` 内保存 `id: todo_<uuid>`，普通新项使用 UUID4，身份与文本、attribution 分离。`todos` 仍是按文本去重的 `list[str]`；不同 ID 可以共享文本。`todos(include_provenance=True)` 按身份 additive 返回 `todo_id`，legacy 无 ID 项显示 `null`，默认输出不变。读取和无关 metadata 更新不补历史 ID，不做批量迁移。
- `trace(bucket_id=..., todo_done="todo_<uuid>")` — 用于婷明确表示事项已经完成。单条待办完成，必须独立调用，只允许同时传 `bucket_id`、`todo_done` 和确认时的 `confirm_token`；不支持文本、数组下标、批量完成、reopen/undo，也不为 legacy 项补 ID。首次调用仅预览 bucket、todo ID、文本、pending 状态和完成动作，返回 5 分钟一次性 token，不写状态。使用同一目标和 token 才完成；bucket 不会自动 resolved。
...
- `trace(bucket_id=..., todo_drop="todo_<uuid>")` — 仅用于婷明确表示该事项不再做、取消或放弃；模型不能因为事项很旧、importance 低或长期未完成自行 drop。只允许显式传 `bucket_id`、`todo_drop`、`confirm_token`，额外参数即使为默认值也拒绝。首次返回 active 状态、放弃动作及 5 分钟一次性确认 token，不写状态；同一目标回传 token 才提交。放弃不等于完成，不删除文本、身份或历史，不自动 resolved，也不改变 bucket 生命周期。
- drop 保存在 `todo_provenance.dropped_at`，由服务端在第一次成功提交时用现有时间 helper 生成。外部 `todo_items` 禁止指定 `done_at` 或 `dropped_at`，包括 null。内部旧 payload 的 missing/null 不能清除既有终态时间；相同非空值可合并，不同非空值冲突。DONE 与 DROPPED 严格互斥，已完成不能再放弃，已放弃不能再完成；本 Phase 不提供 reopen/restore/undo、批量放弃、理由或 legacy ID repair。
- 已放弃再次 drop（包括携带旧 token）直接返回原 `dropped_at`，不消费 token、不刷新、不重复写入。确认在共享 writer mutex 内重读及终态校验后才消费 token，再原子发布；消费后发布失败保持原文件及 active 状态，旧 token 不可重用，须重新 preview。成功发布后响应丢失可安全重试。done/drop 的 confirmation 同时绑定两个状态时间及整个 canonical todo/provenance 集合，正文、importance、tags 等无关 metadata 不使 token stale。
- active todo 统一为 `done_at` 与 `dropped_at` 都为空。`todos()` 和 talk/code/tg boot 共用 canonical projection，排除两种终态；T、rotation 候选与 S/hidden/remaining 自动反映 active 集合。same-text 多 identity 独立判断，legacy 无 ID 仍 active 且读取不补 ID；不能单条放弃时返回“该 todo 为旧格式，无稳定 ID，当前不能单条放弃。”。reconciliation、replacement、attribution、merge、durable import/replay 保留 completed/dropped 历史，双终态即使在 permissive 读取下也明确报冲突。
...
- `todos()` 的活动投影与持久化分离：同文本只要有任一 active ID 或无 ID legacy 身份就继续活动；只有全部可识别身份均已完成或放弃且没有 legacy active 身份时才隐藏。provenance 模式只列活动身份。完成或放弃不会删除 `todos: list[str]` 中的文本；boot 的活动待办展示复用同一 active projection。
...
- todo reconciliation — 同 ID 合并为一个身份，不同 ID 即使文本相同也分别保留。重排、重复复用、update 和 merge 保留现有 ID，merge 后 source 删除不影响迁入的身份；不做全库文本 identity dedupe。明确按 ID 改写文本保留身份；未指明身份的文本改写按替换语义创建新项，移除文本裁掉 active sidecar，completed/dropped 身份及首次 `done_at`/`dropped_at` 继续保留为历史。attribution 相同则保留、known 胜过 unknown、同身份的 known 冲突降为 unknown，但不会因此改写或删除 ID 或终态。
...
- durable import — operation plan 首次创建时固定 ID，并纳入 payload/digest；apply、retry 和 restart 复用原计划。已有目标身份优先复用，计划已提交但 item checkpoint 尚未写入时也不重新规划。旧格式 operation 保留原 payload/digest，在明确 todo 写入时建立缺失身份，重放不更换已落盘 ID。旧计划重放保留已有终态和首次 `done_at`/`dropped_at`，不重新激活 completed/dropped 身份；启动展示继续使用 canonical active projection。
- provenance-only metadata updates 不新增 boot-delta 事件；现有 delta 继续只报告 todo 文本的开闭变化 / Provenance-only metadata changes intentionally do not create boot-delta events; existing deltas remain text-change-only.
...
- `pinned` 与 `permanent` 是独立状态，但保持 `pinned => permanent`。`pinned=0` 只取消开机常驻，仍为 permanent；`permanent=-1` 不改、`1` 升为 permanent、`0` 降为 dynamic。已 pinned 时单独 `permanent=0` 拒绝；`pinned=0, permanent=0` 可通过同一个确认计划降级。保护状态降低沿用限时、一次性确认。 / Unpinning alone does not demote the lifecycle type; combined unpin and demotion uses one confirmation.
...
- Dashboard 详情页 `DELETE /api/bucket/{id}` 与导入审阅 `action="delete"` 复用上述服务端删除计划。首次请求返回目标、影响与 5 分钟 confirm_token，不执行删除；第二次请求在 JSON 中提交 `confirm_token`。sealed/pinned/protected 与 incoming `superseded_by` 一律阻断，JS 弹窗不能越过保护。预览可报告 incoming `related_buckets`，现有删除流程不清理这些关系；related 一致性另属 W-8。
- `trigger_date: str = ""` — 空字符串不改；`YYYY-MM-DD` 设置日期；`trace(trigger_date="none")` 清除日期。到期后由 `boot` 的“今日浮现”显示，现有每日 seen 状态仍在 profiles 间共享 / Blank preserves, a date sets, and `none` clears the trigger date.
- `provenance_kind: str = ""` — 仅单桶创建或 `trace` 可显式断言 `unknown`、`summary`、`inference` 或 `system`；空值表示没有显式断言。无效显式值会被拒绝，旧桶缺失或损坏值在读取时 fail closed 为 `unknown`，且不会改写旧 frontmatter。正文替换/追加会在没有显式新值时重置为 `unknown`；元数据-only 修改保持原值；merge 拼接正文始终变为 `unknown`。批量 `trace` 不支持此字段 / Only single-bucket creation or `trace` may explicitly assert one of the four canonical values. Blank means no assertion. Invalid public values are rejected; missing or corrupted legacy frontmatter reads fail closed as `unknown` without rewriting it. Body replacement/append resets to `unknown` unless explicitly reclassified; metadata-only changes preserve it; a body-concatenating merge always becomes `unknown`. Batch `trace` rejects it.

#### `todos`

- `todos(include_provenance=False)` 保持既有的按 bucket 汇总输出。仅传 `include_provenance=True` 时，才在这个专用 todo 输出中按 `ting`、`model`、`system`、`unknown` 分组；不会改变 `boot`、TG boot、`breath`、`pulse` 或 `dream` 输出。缺失或 malformed 的 sidecar 一律按 unknown 读取，且不会触发写入 / Keeps the existing bucket-grouped output by default. Only the opt-in dedicated todo output groups explicit provenance; other readouts remain unchanged. Missing or malformed sidecars are read as unknown and never trigger a write.
...
| `merge_threshold` | O5B 更新规划专用相似度阈值；普通 hold/grow 和 generic import 不使用此阈值，仅复用确定性文本重复项 / O5B-only similarity threshold for update planning; ordinary hold/grow and generic imports do not use it and reuse only deterministic textual duplicates | `75` |
...
Dashboard 使用独立的密码认证和 setup 流程；Dashboard/API 认证与 MCP 的 Bearer token 是两个不同边界。`OMBRE_TRANSPORT=stdio` 的本地 stdio 启动路径保持不变，不需要 HTTP MCP token。
Dashboard uses a separate password and setup flow; Dashboard/API authentication and the MCP Bearer token are separate boundaries. The local `OMBRE_TRANSPORT=stdio` startup path is unchanged and does not require an HTTP MCP token.
...
5. **验证 / Verify**
   ...
   - 最终 MCP 地址 / MCP URL：`https://<你的域名>.zeabur.app/mcp`；客户端必须发送 `Authorization: Bearer <your-token>`
   - MCP URL: `https://<your-domain>.zeabur.app/mcp`; clients must send `Authorization: Bearer <your-token>`
