...
- Stage 8G-D adds local-fixture migration acceptance on top of that bounded
  ...
  passed. Reconciliation uses a renewable short coordination freeze; recovery
  diagnostics accept only a matching structured acceptance report and never a
  caller-supplied verified flag. It adds no server, MCP, HTTP, Dashboard,
  CLI, startup, production, runtime-enablement, Reindex, dual-write,
  shadow-write, cleanup, or legacy-deletion path.
